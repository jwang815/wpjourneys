window.__i18n = {
  "wa_nav_expeditions": {
    "en": "Expeditions",
    "zh": "探险之旅"
  },
  "wa_nav_about": {
    "en": "About Us",
    "zh": "关于我们"
  },
  "wa_nav_blog": {
    "en": "Blog",
    "zh": "博客"
  },
  "wa_nav_book": {
    "en": "Book Now",
    "zh": "立即预订"
  },
  "wa_hero_eyebrow": {
    "en": "Waypoint Journeys × West Africa",
    "zh": "Waypoint Journeys × 西非"
  },
  "wa_hero_title": {
    "en": "Ghana, Benin<br>&amp; Togo",
    "zh": "加纳、贝宁<br>与多哥"
  },
  "wa_hero_subtitle": {
    "en": "Voodoo, Castles & the Slave Coast",
    "zh": "巫毒圣地、古堡遗址与奴隶海岸"
  },
  "wa_hero_dates": {
    "en": "8 Days",
    "zh": "8天"
  },
  "wa_hero_tagline": {
    "en": "8 days across three nations at the crossroads of history",
    "zh": "八天穿越三国，踏上历史的交汇之地"
  },
  "wa_hero_scroll": {
    "en": "Scroll",
    "zh": "向下滚动"
  },
  "wa_overview_label": {
    "en": "The Destination",
    "zh": "目的地"
  },
  "wa_overview_title": {
    "en": "Where History Lives in the Streets, the Temples, and the Sea",
    "zh": "历史活在街道、神殿与大海之间"
  },
  "wa_overview_p1": {
    "en": "West Africa’s Gulf of Guinea coast holds some of the most powerful stories on Earth. This is the Slave Coast — where for three centuries, millions were taken through the Door of No Return at Ouidah. It is also the birthplace of Voodoo, where ancient spirits still walk through sacred forests and living temples. And it is modern, vibrant, resilient Africa — market women in Lomé trading in cloth empires, fishermen casting nets at dawn across Lake Nokoué, and Accra’s creative energy reshaping a continent.",
    "zh": "西非几内亚湾海岸承载着世界上最震撼人心的历史篇章。这里是昔日的“奴隶海岸”——三个世纪间，数百万人从维达的“不归之门”被强行带走。这里同样是巫毒教的发源地，古老的神灵至今仍游荡于圣林与活生生的神殿之中。而今，这片土地焕发着蓬勃的现代生机——洛美市场的女商人掌管着布料帝国，渔夫在诺克韦湖的晨曦中撒网，阿克拉涌动的创造力正在重塑整个非洲大陆。"
  },
  "wa_overview_p2": {
    "en": "This eight-day expedition moves through three countries — Benin, Togo, and Ghana — following the thread of history from the stilt villages of Ganvié to the royal palaces of Abomey, from the sacred python temples of Ouidah to the slave castles of Cape Coast and Elmina. Every day brings encounters that no textbook can deliver.",
    "zh": "这趟八天探险跨越贝宁、多哥与加纳三国，沿着历史的脉络一路行进：从甘维耶的高脚屋村落，到阿博美的王宫；从维达神圣的蟒蛇神殿，到海角海岸与埃尔米纳的奴隶城堡。每一天的相遇，都是任何教科书无法给予的深刻体验。"
  },
  "wa_overview_pullquote": {
    "en": "“History lives here — not in museums, but in the streets, the temples, and the ocean that remembers.”",
    "zh": "“历史就活在这里——不在博物馆里，而在街道上、神殿中，以及那片铭记一切的大海深处。”"
  },
  "wa_overview_image_caption": {
    "en": "Stilt Village, Benin",
    "zh": "高脚屋村落，贝宁"
  },
  "wa_highlights_label": {
    "en": "Why West Africa",
    "zh": "为何选择西非"
  },
  "wa_highlights_title": {
    "en": "A Region Unlike Any Other",
    "zh": "一片举世无双的土地"
  },
  "wa_feature1_title": {
    "en": "Birthplace of Voodoo",
    "zh": "巫毒教的发源地"
  },
  "wa_feature1_desc": {
    "en": "Ouidah is where Voodoo began. Visit the Sacred Forest, the Temple of the Pythons, and witness a living spiritual tradition practised by millions across West Africa and the diaspora.",
    "zh": "维达是巫毒教的诞生之地。探访圣林与蟒蛇神殿，亲眼见证这一古老的精神信仰——时至今日，它仍在西非及海外侨民社群中被数百万人虔诚奉行。"
  },
  "wa_feature2_title": {
    "en": "The Slave Coast",
    "zh": "奴隶海岸"
  },
  "wa_feature2_desc": {
    "en": "Walk the Route des Esclaves in Ouidah and stand at the Door of No Return. Visit Cape Coast and Elmina Castles in Ghana — the two most significant slave forts remaining on the African coast.",
    "zh": "沿维达的“奴隶之路”而行，伫立于“不归之门”前。前往加纳的海角城堡与埃尔米纳城堡——这两座非洲海岸保存最完整、历史意义最深重的奴隶要塞。"
  },
  "wa_feature3_title": {
    "en": "Ganvié — Venice of Africa",
    "zh": "甘维耶——非洲的威尼斯"
  },
  "wa_feature3_desc": {
    "en": "Home to 35,000 people living entirely on water, Ganvié is the largest stilt village in Africa. Glide through its channels by pirogue and witness a way of life unchanged for centuries.",
    "zh": "甘维耶是非洲最大的水上高脚屋村落，三万五千名居民世代生活于水面之上。乘坐独木舟穿行于迷宫般的水道之间，亲历一种延续数百年、不曾改变的生活方式。"
  },
  "wa_feature4_title": {
    "en": "Royal Abomey",
    "zh": "王者阿博美"
  },
  "wa_feature4_desc": {
    "en": "The Royal Palaces of Abomey, a UNESCO World Heritage Site, were the seat of the Dahomey Kingdom — one of the most powerful and feared empires in pre-colonial Africa.",
    "zh": "阿博美王宫群被列为联合国教科文组织世界遗产，曾是达荷美王国的权力中心——这是前殖民时代非洲最强盛、最令人敬畏的帝国之一。"
  },
  "wa_feature5_title": {
    "en": "Three Countries, Eight Days",
    "zh": "三国之旅，八天探索"
  },
  "wa_feature5_desc": {
    "en": "Cross two borders and experience three distinct cultures — the Fon traditions of Benin, the Ewe and Mina cultures of Togo, and the Akan and Ga heritage of Ghana.",
    "zh": "两度跨越国境，沉浸于三种截然不同的文化之中——贝宁丰族的古老传统、多哥埃维族与米纳族的活态文化，以及加纳阿坎族与加族的深厚文明遗产。"
  },
  "wa_feature6_title": {
    "en": "Markets & Living Culture",
    "zh": "集市与鲜活的文化"
  },
  "wa_feature6_desc": {
    "en": "From the fetish market of Lomé to the Grand Marché, from the artisan villages of Togo to the Makola Market of Accra — West Africa’s markets are theatres of daily life.",
    "zh": "从洛美的巫术市场到大集市，从多哥的手工艺人村落到阿克拉的马科拉市场——西非的集市，是日常生活最生动的舞台。"
  },
  "wa_itinerary_label": {
    "en": "Day by Day",
    "zh": "行程日志"
  },
  "wa_itinerary_title": {
    "en": "The Expedition",
    "zh": "探险全程"
  },
  "wa_itinerary_subtitle": {
    "en": "Eight days of discovery across three nations at the crossroads of history.",
    "zh": "八天深度探索，穿越历史交汇的三国大地。"
  },
  "wa_day1_label": {
    "en": "Day 1",
    "zh": "第一天"
  },
  "wa_day1_location": {
    "en": "Arrival in Cotonou",
    "zh": "抵达科托努"
  },
  "wa_day1_date": {
    "en": "Day 1",
    "zh": "第1天"
  },
  "wa_day1_desc": {
    "en": "Arrive at Cotonou Cadjehoun Airport, Benin’s economic capital on the Gulf of Guinea. Transfer to your hotel in Ouidah, a historic town on the coast that serves as your base for the first three nights. Settle in and prepare for a week that will reshape how you see Africa.",
    "zh": "抵达科托努卡杰胡恩机场——贝宁的经济首都，坐落于几内亚湾畔。驱车前往沿海历史名城维达入住酒店，这里将是最初三晚的根据地。安顿下来，静候一段将彻底改变你对非洲认知的旅程。"
  },
  "wa_day2_label": {
    "en": "Day 2",
    "zh": "第二天"
  },
  "wa_day2_location": {
    "en": "Ganvié & Abomey",
    "zh": "甘维耶与阿博美"
  },
  "wa_day2_date": {
    "en": "Day 2",
    "zh": "第2天"
  },
  "wa_day2_desc": {
    "en": "Depart Ouidah early for Ganvié, the extraordinary stilt village on Lake Nokoué — home to 35,000 people and known as the Venice of Africa. Glide through its channels by pirogue past homes, markets, and schools built entirely on stilts. Continue to Abomey to visit the Royal Palace Museum, a UNESCO World Heritage Site and seat of the once-mighty Dahomey Kingdom. Explore the underground village of Bohicon and the Chameleon Temple before returning to Ouidah for the night.",
    "zh": "清晨离开维达，前往诺克韦湖上的甘维耶——这座令人叹为观止的高脚屋村落拥有三万五千名居民，素有“非洲威尼斯”之称。乘独木舟轻轻滑过水道，掠过一栋栋建于水面的民居、市集与学校。随后驱车前往阿博美，参观联合国教科文组织世界遗产阿博美王宫博物馆，探寻昔日达荷美王国的辉煌遗迹。游览博希孔的地下村落与变色龙神殿后，返回维达共度夜晚。"
  },
  "wa_day3_label": {
    "en": "Day 3",
    "zh": "第三天"
  },
  "wa_day3_location": {
    "en": "Ouidah — Birthplace of Voodoo",
    "zh": "维达——巫毒教的圣地"
  },
  "wa_day3_date": {
    "en": "Day 3",
    "zh": "第3天"
  },
  "wa_day3_desc": {
    "en": "Ouidah is the spiritual heartland of Voodoo, a living faith practised by millions. Walk the Route des Esclaves, the path slaves were forced to march to the sea, passing through the Sacred Forest where annual Voodoo Festivals draw devotees from around the world. Visit the Cathedral that stands alongside the Temple of the Sacred Pythons — a sanctuary where dozens of pythons roam freely as objects of worship. End the day at the haunting Gate of No Return, the monument marking where an estimated 12 million enslaved people departed these shores between the 16th and 19th centuries.",
    "zh": "维达是巫毒教的精神圣地，这一活态信仰至今仍被数百万信徒虔诚奉行。沿“奴隶之路”而行——这是昔日奴隶被驱赶入海的路途，途经圣林，每年的巫毒教节庆便在此处吸引世界各地的信众前来朝圣。参观大教堂旁的圣蛇神殿，那里数十条蟒蛇自由游走，被视为神灵的化身。在令人动容的“不归之门”纪念碑前结束这一天——这座纪念碑铭记着16世纪至19世纪间约1200万名被奴役者从此离岸，一去不返。"
  },
  "wa_day4_label": {
    "en": "Day 4",
    "zh": "第四天"
  },
  "wa_day4_location": {
    "en": "Lomé, Togo",
    "zh": "洛美，多哥"
  },
  "wa_day4_date": {
    "en": "Day 4",
    "zh": "第4天"
  },
  "wa_day4_desc": {
    "en": "Cross the border into Togo after an early morning departure. After a ninety-minute drive and immigration formalities, begin exploring Lomé, Togo’s low-key coastal capital. Visit a preserved slave house in Agbodrafo, then take in Independence Square, the Artisans Village, and the National Museum. The afternoon is devoted to the Fetish Market — the largest traditional medicine market in West Africa — followed by the fishing harbour, Art Street, and the bustling Grand Marché. Overnight in Lomé.",
    "zh": "清晨启程，越过边境进入多哥。经过九十分钟的车程与入境手续后，开始探索洛美——多哥低调而独特的海滨首都。前往阿格博德拉福参观保存完好的奴隶旧居，随后游览独立广场、手工艺人村与国家博物馆。下午全程留给巫术市场——西非规模最大的传统草药市集——继而漫步渔港、艺术街，最终融入人声鼎沸的大集市。夜宿洛美。"
  },
  "wa_day5_label": {
    "en": "Day 5",
    "zh": "第五天"
  },
  "wa_day5_location": {
    "en": "Drive to Accra, Ghana",
    "zh": "驱车前往阿克拉，加纳"
  },
  "wa_day5_date": {
    "en": "Day 5",
    "zh": "第5天"
  },
  "wa_day5_desc": {
    "en": "Depart Lomé for the Ghana border — just ten minutes from the hotel. After immigration protocols, begin the five-hour drive to Accra along the coastal road. Arrive in the afternoon and check into your hotel for an evening of rest and relaxation before two full days of exploration.",
    "zh": "从洛美出发，仅十分钟便可抵达加纳边境。完成入境手续后，沿海岸公路开始五小时的车程，前往阿克拉。下午抵达入住酒店，在两天深度探索开始之前，享受一个悠然自得的夜晚。"
  },
  "wa_day6_label": {
    "en": "Day 6",
    "zh": "第六天"
  },
  "wa_day6_location": {
    "en": "Accra City Tour",
    "zh": "阿克拉城市探索"
  },
  "wa_day6_date": {
    "en": "Day 6",
    "zh": "第6天"
  },
  "wa_day6_desc": {
    "en": "A full day immersed in Ghana’s vibrant capital. Visit the Kwame Nkrumah Memorial Park, dedicated to the founding father of Ghanaian independence. Stand at Independence Square, also known as Black Star Square, the symbolic heart of the nation. Tour the National Museum, drive past the imposing Osu Castle — the former seat of the Ghanaian presidency — and walk through the historic streets of Jamestown, Accra’s oldest neighbourhood. In the afternoon, drive to Cape Coast to position for the following day.",
    "zh": "整日沉浸于加纳生机勃勃的首都之中。参观恩克鲁玛纪念公园，向加纳独立之父致敬；伫立于象征国家精神的独立广场——亦称“黑星广场”——感受这片土地的历史重量。游览国家博物馆，驶过气势磅礴的奥苏城堡——昔日的加纳总统府——并漫步于詹姆斯敦古朴的街巷，探访阿克拉最古老的街区。下午驱车前往海角海岸，为翌日的行程做好准备。"
  },
  "wa_day7_label": {
    "en": "Day 7",
    "zh": "第七天"
  },
  "wa_day7_location": {
    "en": "Cape Coast & Elmina",
    "zh": "海角海岸与埃尔米纳"
  },
  "wa_day7_date": {
    "en": "Day 7",
    "zh": "第7天"
  },
  "wa_day7_desc": {
    "en": "A day of profound historical weight. Visit Cape Coast Castle, the largest of the slave forts built along the Gold Coast, where enslaved people were held in underground dungeons before being shipped across the Atlantic. Continue to Elmina Castle, the oldest European building in sub-Saharan Africa, built by the Portuguese in 1482. Both are UNESCO World Heritage Sites. The experience is sobering, powerful, and essential. Return to Accra in the evening.",
    "zh": "这是承载着沉重历史的一天。参观海角城堡——黄金海岸最大的奴隶要塞，被奴役者曾被关押于地下牢狱，等待被运往大西洋彼岸的命运。随后前往埃尔米纳城堡，这座由葡萄牙人于1482年修建的建筑，是撒哈拉以南非洲现存最古老的欧洲建筑。两座城堡均为联合国教科文组织世界遗产。这段经历沉重、震撼，却是这趟旅程中不可或缺的核心时刻。傍晚返回阿克拉。"
  },
  "wa_day8_label": {
    "en": "Day 8",
    "zh": "第八天"
  },
  "wa_day8_location": {
    "en": "Departure from Accra",
    "zh": "从阿克拉启程返航"
  },
  "wa_day8_date": {
    "en": "Day 8",
    "zh": "第8天"
  },
  "wa_day8_desc": {
    "en": "A final morning in Accra before transfers to Kotoka International Airport for your departure. Eight days, three countries, centuries of history, and a journey that will stay with you forever.",
    "zh": "在阿克拉度过最后一个清晨，随后前往科托卡国际机场踏上归途。八天、三国、数百年的历史——这段旅程，将永远留存于你的记忆深处。"
  },
  "wa_cinematic_label": {
    "en": "Small Group Expedition",
    "zh": "小团精品探险"
  },
  "wa_cinematic_title": {
    "en": "Every Detail Arranged.<br><em>Every Moment Yours.</em>",
    "zh": "每一处细节，皆已妥善安排。<br><em>每一个瞬间，皆属于你。</em>"
  },
  "wa_details_label": {
    "en": "Expedition Details",
    "zh": "行程详情"
  },
  "wa_details_title": {
    "en": "What’s Included",
    "zh": "费用包含"
  },
  "wa_details_duration_label": {
    "en": "Duration",
    "zh": "行程时长"
  },
  "wa_details_duration_value": {
    "en": "8 days / 7 nights",
    "zh": "8天7晚"
  },
  "wa_details_groupsize_label": {
    "en": "Group Size",
    "zh": "团队规模"
  },
  "wa_details_groupsize_value": {
    "en": "Small group expedition: intimate, unhurried, and personal",
    "zh": "小团精品探险：亲密私享，从容不迫，量身定制"
  },
  "wa_details_countries_label": {
    "en": "Countries",
    "zh": "途经国家"
  },
  "wa_details_countries_value": {
    "en": "Benin, Togo, Ghana",
    "zh": "贝宁、多哥、加纳"
  },
  "wa_details_accommodation_label": {
    "en": "Accommodation",
    "zh": "住宿安排"
  },
  "wa_details_accommodation_value": {
    "en": "Hotels in Ouidah (3 nights), Lomé (1 night), Accra (3 nights)",
    "zh": "维达酒店（3晚）、洛美酒店（1晚）、阿克拉酒店（3晚）"
  },
  "wa_details_meals_label": {
    "en": "Meals",
    "zh": "餐饮"
  },
  "wa_details_meals_value": {
    "en": "All meals included",
    "zh": "全程餐饮费用全包"
  },
  "wa_details_transport_label": {
    "en": "Transport",
    "zh": "交通"
  },
  "wa_details_transport_value": {
    "en": "Private vehicle with driver throughout all ground transfers",
    "zh": "全程配备专属私家车及司机，承担所有陆地交通接送"
  },
  "wa_details_notincluded_label": {
    "en": "Not Included",
    "zh": "费用不含"
  },
  "wa_details_notincluded_value": {
    "en": "International flights, visa fees, travel insurance",
    "zh": "国际机票、签证费及旅行保险"
  },
  "wa_pricing_label": {
    "en": "Expedition Investment",
    "zh": "探险投资"
  },
  "wa_pricing_amount": {
    "en": "$2,950",
    "zh": "$2,950"
  },
  "wa_pricing_per": {
    "en": "USD per person",
    "zh": "美元/每人"
  },
  "wa_pricing_includes": {
    "en": "Fully inclusive of accommodation, activities, and ground transportation",
    "zh": "全包含住宿、活动及地面交通费用"
  },
  "wa_pricing_note": {
    "en": "International airfare and visa fees not included",
    "zh": "国际机票及签证费用不包含在内"
  },
  "wa_pricing_cta": {
    "en": "Reserve Your Spot",
    "zh": "立即预订名额"
  },
  "wa_contact_email": {
    "en": "info@wpjourneys.com",
    "zh": "info@wpjourneys.com"
  },
  "wa_contact_whatsapp": {
    "en": "WhatsApp",
    "zh": "WhatsApp"
  },
  "wa_safety_title": {
    "en": "A Note on Safety",
    "zh": "安全须知"
  },
  "wa_safety_desc": {
    "en": "West Africa is one of the continent’s most welcoming and accessible regions for international visitors. The countries on this route — Benin, Togo, and Ghana — are politically stable and accustomed to hosting travellers. Ghana in particular is one of Africa’s safest destinations, with a strong tradition of hospitality. Our local guides have deep relationships throughout every community on this route, and we coordinate all border crossings and logistics so you can focus entirely on the experience.",
    "zh": "西非是非洲大陆对国际游客最为友好、最易于到访的地区之一。本线路途经的贝宁、多哥与加纳三国政治稳定，素有接待旅行者的丰富经验。其中加纳更是非洲最安全的旅行目的地之一，以深厚的待客之道著称。我们的当地导游与沿途每一个社区都建立了深厚的信任关系，所有边境过境手续及后勤事务均由我们全程统筹安排，让您得以全然专注于旅途体验本身。"
  },
  "wa_footer_tagline": {
    "en": "Luxury adventure travel to the world’s last wild places.",
    "zh": "奢华探险，通往世界最后的荒野秘境。"
  },
  "wa_footer_copy": {
    "en": "© 2026 Waypoint Journeys. All rights reserved.",
    "zh": "© 2026 Waypoint Journeys. 版权所有。"
  },
  "wa_faq_label": {
    "en": "FREQUENTLY ASKED QUESTIONS",
    "zh": "常见问题"
  },
  "wa_faq_title": {
    "en": "Common Questions About This Expedition",
    "zh": "西非探险常见问题"
  },
  "wa_faq_q1": {
    "en": "Is West Africa safe for tourists?",
    "zh": "西非对游客安全吗？"
  },
  "wa_faq_a1": {
    "en": "Ghana, Benin, and Togo are among the most stable and welcoming countries in West Africa. Ghana consistently ranks as one of the safest nations on the continent. Our local team has deep roots in all three countries and manages logistics, security, and cultural access throughout the journey.",
    "zh": "加纳、贝宁和多哥是西非最稳定、最友好的国家之一。加纳一直被评为非洲最安全的国家之一。我们的当地团队在三国都有深厚根基，全程负责后勤、安全和文化访问。"
  },
  "wa_faq_q2": {
    "en": "What is the Voodoo experience like?",
    "zh": "伏都教体验是什么样的？"
  },
  "wa_faq_a2": {
    "en": "Voodoo (Vodun) is a living religion in Benin, not a Hollywood caricature. You\'ll visit sacred temples in Ouidah — the spiritual birthplace of Voodoo — meet practitioners, and witness ceremonies with deep respect and local guidance. It\'s one of the most culturally profound experiences we offer.",
    "zh": "伏都教（Vodun）在贝宁是活生生的宗教，而非好莱坞的刻板印象。您将参访伏都教精神发源地维达的神圣寺庙，会见修行者，在当地向导的引领下以深切敬意观礼仪式。这是我们提供的最深刻的文化体验之一。"
  },
  "wa_faq_q3": {
    "en": "What vaccinations do I need for West Africa?",
    "zh": "前往西非需要哪些疫苗接种？"
  },
  "wa_faq_a3": {
    "en": "Yellow fever vaccination is required for entry to all three countries. We also recommend being up to date on hepatitis A, typhoid, and routine vaccinations. Malaria prophylaxis is strongly recommended. We\'ll send you a complete health brief with specific medical guidance once you book.",
    "zh": "进入三国均需要黄热病疫苗接种。我们还建议更新甲型肝炎、伤寒及常规疫苗。强烈建议服用疟疾预防药物。预订后我们将向您发送包含具体医疗建议的完整健康指南。"
  },
  "wa_faq_q4": {
    "en": "What will I learn about the history of the slave trade?",
    "zh": "我能了解到哪些奴隶贸易历史？"
  },
  "wa_faq_a4": {
    "en": "The expedition includes visits to Cape Coast Castle and Elmina Castle in Ghana — two of the most significant slave trade sites in the world. You\'ll walk through the Door of No Return and learn the full history from expert local guides. In Ouidah, Benin, you\'ll trace the Route of the Slaves. These are deeply moving, essential historical experiences.",
    "zh": "探险包括参访加纳的海岸角城堡和埃尔米纳城堡——世界上最重要的奴隶贸易遗址之二。您将走过\'不归之门\'，由当地专家向导讲述完整历史。在贝宁维达，您将追溯奴隶之路。这些是令人深受触动的重要历史体验。"
  },
  "wa_related_label": {
    "en": "EXPLORE MORE EXPEDITIONS",
    "zh": "探索更多探险之旅"
  },
  "wa_related_title": {
    "en": "Continue Your Journey",
    "zh": "继续您的探险旅程"
  },
  "wa_related_sc_name": {
    "en": "Socotra Island",
    "zh": "索科特拉岛"
  },
  "wa_related_sc_sub": {
    "en": "The Alien Island",
    "zh": "来自另一星球的秘岛"
  },
  "wa_related_mn_name": {
    "en": "Mongolia",
    "zh": "蒙古"
  },
  "wa_related_mn_sub": {
    "en": "Land of the Eternal Sky",
    "zh": "永恒苍穹之地"
  },
  "wa_related_pk_name": {
    "en": "Pakistan",
    "zh": "巴基斯坦"
  },
  "wa_related_pk_sub": {
    "en": "The Karakoram and Ancient Empires",
    "zh": "喀喇昆仑与古老帝国"
  },
  // ── Inquiry CTAs (added 2026-06-09) ──
  "wp_cta_plan":       { "en": "Plan This Expedition →", "zh": "定制此次探险 →" },
  "wp_cta_reply":      { "en": "We reply within the hour", "zh": "我们将在一小时内回复" },
  "wp_cta_sticky":     { "en": "Request Itinerary", "zh": "索取行程方案" },
  "wp_cta_sticky_sub": { "en": "We reply within the hour", "zh": "一小时内回复" }
};
